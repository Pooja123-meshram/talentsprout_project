...
from examination.models import Test  # ✅ Make sure this import is added if not already(if user passed the exam)
...
def all_posts(request):
    posts_list = BlogPost.objects.filter(status='published').order_by('-publication_date')  # Fetch all posts
...
def create_blog_post(request):
    ...
    if not has_passed_exam(request.user):
        return render(request, 'studentPost/access_denied.html', {
            'message': "You must pass the exam to create blog posts."
        })
...
# Utility function to check if user passed the exam
def has_passed_exam(user):
    latest_score = Test.fetch_latest_score(user)
    return latest_score is not None and latest_score >= 60