...
def talent_view(request):
    ...
    # Random skill just for showing some "random skill" stats text or contracts count
    random_skill = None  # Set to None for initial page load

    
    # Initially show ALL services and ALL user profiles with profile images (not filtered)
    related_services = Service.objects.all()
    related_user_profiles = UserProfile.objects.filter(profile_image__isnull=False).exclude(profile_image='')
    ...
    context = {
        'random_skill': random_skill.skill if random_skill else '',
        'related_services': related_services,
        'related_user_profiles': related_user_profiles,
        'rest_of_skills': all_skills,
        'total_skills': total_skills,
        'random_contracts': f"{random.randint(10, 50)}+",
        'profile_image_url': profile_image_url,
    }
