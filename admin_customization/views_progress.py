...
from progress_tracker.models import Project ,ProjectDetails
...
@login_required
def admin_project_progress_details_view(request, project_id):
    project = get_object_or_404(ProjectDetails, id=project_id)
    stages = project.stages.all().order_by('updated_at')  # from related_name='stages'

    return render(request, 'admin_customization/project_progress/admin_project_progress.html', {
        'project': project,
        'stages': stages
    })
