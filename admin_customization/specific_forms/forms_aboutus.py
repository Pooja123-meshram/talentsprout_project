...
class AboutUsContentForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['title', 'description', 'image','mission_title','mission_description']
        widgets = {
            'title': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Enter title'}),
            'description': TinyMCE(attrs={'class': 'form-control', 'rows': 3}),
            'image': forms.ClearableFileInput(attrs={'class': 'form-control-file'}),
            'mission_title': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Enter mission title'}),
            'mission_description': TinyMCE(attrs={'class': 'form-control', 'rows': 3}),
        }
...
class SupportInfoForm(forms.ModelForm):
    class Meta:
        model = SupportInfo
        fields = ['title', 'description', 'phone', 'email', 'address', 'support_image', 'contatus_image', 'map_title', 'map_description', 'map_embed_url']
        widgets = {
            'title': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Enter title'}),
            ...
            'map_title': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Enter map section title (e.g., Find Us on Google Maps)'}),
            'map_description': forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'Enter short description under the map title'}),
            'map_embed_url': forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'Paste Google Maps embed URL'}),
        }
