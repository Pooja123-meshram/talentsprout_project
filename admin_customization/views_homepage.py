...
def add_why_choose_us(request):
    if request.method == 'POST':
        form = WhyChooseUsForm(request.POST)
        if form.is_valid():
            form.save()
            messages.success(request, 'Why Choose Us section added successfully!')
            return redirect(reverse('admin_home_page'))
    else:
        form = WhyChooseUsForm()

    return render(request, 'admin_customization/homepage/add_why_choose_us.html', {
        'site_header': "Add Why Choose Us Content",
        'form': form,
    })

@login_required
@admin_required
def edit_why_choose_us(request, id):
    section = get_object_or_404(WhyChooseUs, id=id)
    if request.method == 'POST':
        form = WhyChooseUsForm(request.POST, instance=section)
        if form.is_valid():
            form.save()
            return redirect(reverse('admin_home_page'))
    else:
        form = WhyChooseUsForm(instance=section)

    return render(request, 'admin_customization/homepage/edit_why_choose_us.html', {
        'site_header': "Edit Why Choose Us Content",
        'form': form,
    })
...
def delete_why_choose_us(request, id):
    ...
    if request.method == 'POST':
        section.delete()
        messages.success(request, 'Why Choose Us section deleted successfully!')
        return redirect(reverse('admin_home_page'))
