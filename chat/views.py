...
from progress_tracker.forms import ProjectDetailsForm
...
def chat_session(request, chat_session_id):
    ...
    # ).order_by('timestamp')
    messages = Message.objects.filter(
    (Q(sender=user, receiver=other_participant) |
     Q(sender=other_participant, receiver=user))
    ).order_by('timestamp')
...
@login_required
def assign_project_view(request, candidate_id):
    candidate = get_object_or_404(CustomUser, id=candidate_id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            project.recruiter = request.user
            project.candidate = candidate
            project.save()
            messages.success(request, "Project assigned successfully.")
            return redirect('subscription_list', candidate_id)  # 👈 Use this instead

    # fallback redirect if GET or invalid
    return redirect('subscription_list', candidate_id)
